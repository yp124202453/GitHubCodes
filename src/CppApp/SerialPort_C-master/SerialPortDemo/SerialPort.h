...
public:
	...
	*/
	...
	//读取缓冲区指定长度字节
	bool ReadBytes(byte* buffe,UINT byteToread);

	bool DetectPort(UINT PortNo);
...
private:
	...
	/*vector<void(*)(byte*, UINT)> funcs;*/

	function<void(byte*, UINT)> m_funcs;

	/*typedef void(*FUNCS)(byte*, UINT, void* user);
	FUNCS BACK;
	void* m_user;*/
...
public:
	...
	/*void registerCallback(void(*p)(byte*, UINT));*/
	
	void CallBack(function<void(byte*, UINT)> param);
